...
import { searchNotes } from "../../services/vault/vault-service.js";
...
export class KnowledgeSubAgent extends BaseSubAgent {
  ...
  async _doWork(instruction, context = {}) {
    ...
    try {
      ...
      // Step 1: Search knowledge base (DB docs + vault notes)
   ...
   * Combines legacy KnowledgeDocument records with vault notes (if workspace available).
   */
  async _searchDocuments({ query, userId, workspaceId, agentId }) {
    ...
    // Vault notes — append when a workspaceId is available
    if (workspaceId) {
      try {
        const vaultResults = await searchNotes(workspaceId, query, this.searchLimit);
        ...
        // Vault pas encore initialisé ou workspace sans vault — ignorer
      }
    }

    return scored.sort((a, b) => b.relevanceScore - a.relevanceScore).slice(0, this.searchLimit * 2);
  }
