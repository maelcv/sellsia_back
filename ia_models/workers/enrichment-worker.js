...
export async function enrichPendingSiret() {
  try {
    let pending;
    try {
      pending = await prisma.siretEnrichment.findMany({
      ...
      if (err?.message?.includes("Tenant or user not found") || err?.message?.includes("FATAL")) {
        console.log(`[SIRENE Worker] Skipping (no tenant context)`);
        return;
      }
      throw err;
    }
    ...
    if (!pending || pending.length === 0) {
      console.log(`[SIRENE Worker] No pending enrichments`);
      return;
    }
    ...
    console.error("[SIRENE Worker] Error:", err.message);
  }
}
...
export function startEnrichmentWorker() {
  ...
  setTimeout(() => {
    enrichPendingSiret().catch(err => console.error("[SIRENE Worker] Error on startup:", err.message));
  }, 5000);

  // Then every hour
  setInterval(() => {
    enrichPendingSiret().catch(err => console.error("[SIRENE Worker] Error on interval:", err.message));
  }, 60 * 60 * 1000);
